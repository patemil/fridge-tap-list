...
impl<I: Write> LTC2633<I> {
    ...
    pub fn select_internal_vref (&mut self) -> Result<(), I::Error> {
        self.device.write(LTC2633_I2CADDR, &[CMD_SELECT_INTERNAL_VREF, 0x0, 0x0]) ?;
        Ok(())
    }

    pub fn write_to_and_update_a(&mut self, val: u16) -> Result<u16, I::Error> {
        self.device.write(LTC2633_I2CADDR, &[CMD_WRITE_TO_AND_UPDATE_DACA, (val >> 4) as u8, (val << 4) as u8])?;
        Ok(val)
    }

    pub fn write_to_and_update_b(&mut self, val: u16) -> Result<u16, I::Error> {
        self.device.write(LTC2633_I2CADDR, &[CMD_WRITE_TO_AND_UPDATE_DACB, (val >> 4) as u8, (val << 4) as u8])?;
        Ok(val)
    }
