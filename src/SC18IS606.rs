...
impl<I: Write + Read + WriteRead> SC18IS606<I> {
    ...
    pub fn read_version(&mut self) -> Result<[u8; 16], <I as WriteRead>::Error> {
        let mut data = [0u8; 16];
        self.device
            .write_read(SC18IS606_I2CADDR, &[FUNCID_VERSION], &mut data)?;
        Ok(data)
    }
    ...
    pub fn read_rega1(&mut self) -> Result<(), <I as Write>::Error> {
        self.device.write(
            SC18IS606_I2CADDR,
            &mut [0x1, 0x80, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
        )?; // read register
    ...
    pub fn read_rega2(&mut self) -> Result<(), <I as Read>::Error> {
        self.device
            .read(SC18IS606_I2CADDR, &mut [0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0])?; // read register
        Ok(())
    }

    pub fn setgaina(&mut self, gain: u8) -> Result<(), <I as Write>::Error> {
        self.device
            .write(SC18IS606_I2CADDR, &[FUNCID_SS0, 0x2, gain])?;
        Ok(())
    }

    pub fn setgainb(&mut self, gain: u8) -> Result<(), <I as Write>::Error> {
        self.device
            .write(SC18IS606_I2CADDR, &[FUNCID_SS1, 0x2, gain])?;
        Ok(())
    }
    ...
    pub fn read_u16(&mut self, reg: u8) -> Result<u16, <I as WriteRead>::Error> {
        let mut buf = [0u8; 2];
        self.device
            .write_read(SC18IS606_I2CADDR, &[reg], &mut buf[..])?;
